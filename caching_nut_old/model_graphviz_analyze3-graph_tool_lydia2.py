...
# N.B.: removed this check, since it slows things down by a factor of ~5-10!
...
# N.B.: using integer indices instead of full vertex objects reduces runtime
#       with another factor ~4!
# nodes_queue = [int(v) for v in nodes_queue]
nodes_queue = get_children_ix(root)

# empty the queue!
with tqdm.tqdm(total=G.num_vertices()) as pbar:
    while nodes_queue:
        ...
        try:
            ...
            # parents = [int(v) for v in get_parents(G.vertex(node))]
            parents = get_parents_ix(G.vertex(node))
            N_calls_node = 0
            for parent in parents:
                ...
                try:
                    ...
                    N_calls_node += N_calls_array[parent]
                except KeyError:
            ...
            N_calls_array[node] = N_calls_node
            # definitively remove node from pile
            # all_nodes_pile.remove(node)  # TOO SLOW
            # put node's children at the end of the queue (= beginning of list)
            # children = [int(v) for v in get_children(G.vertex(node))]
            children = get_children_ix(G.vertex(node))
