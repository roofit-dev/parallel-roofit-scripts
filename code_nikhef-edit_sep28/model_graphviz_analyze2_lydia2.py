...
G = nx_pydot.read_dot(dn + "lydia2.dot")
...
while nodes_queue:
    ...
    try:
        ...
        nodes_queue = list(more_itertools.unique_everseen(G.successors(node) +
                                                          nodes_queue))
...
fundamental_type_nodes = zip(*filter(lambda n: n[1]['color'] == 'blue',
                                     G.node.iteritems()))[0]
non_fundamental_type_nodes = zip(*filter(lambda n: n[1]['color'] == 'red',
                                         G.node.iteritems()))[0]
...
node_types_indegree_not_cacheable = collections.Counter([i.split('_')[-1] for i in nodes_indegree_not_cacheable])
...
# parameters:
nodes_indegree_really_cacheable = [n for n in nodes_indegree_cacheable if n not in lydia1_obs_par.const_params + lydia1_obs_par.observables]
...
nodes_indegree_cacheable_direct = [i for i in nodes_indegree_cached if i in lydia1_obs_par.var_params]
...
server_nodes_cacheable_nodes = {n: zip(*G.out_edges(n)) for n in nodes_indegree_cacheable_direct}
