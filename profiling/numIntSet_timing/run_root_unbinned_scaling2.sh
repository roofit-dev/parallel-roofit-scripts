...
# @Last Modified time: 2017-06-15 08:09:22

#PBS -l nodes=1:ppn=8

if [[ -z "$run_id" || -z "$timing_flag" || -z "$g" || -z "$o" || -z "$p" || -z "$ileave" || -z "$seed" || -z "$printlevel" || -z "$optConst" || -z "$time_num_ints" || -z "$e" || -z "$cpu" || -z "$force_num_int" || -z "$fork_timer" || -z "$fork_timer_sleep_us" || -z "$cpu_affinity" || -z "$debug" ]]; then
...
root -b -q -l "$SCRIPT_PATH(${cpu},${force_num_int},${time_num_ints},${optConst},${g},${o},${p},${e},${ileave},${seed},${printlevel},${timing_flag},${cpu_affinity},${fork_timer},${fork_timer_sleep_us},${debug})"
