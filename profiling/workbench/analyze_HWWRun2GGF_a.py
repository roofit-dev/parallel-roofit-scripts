...
"""
cd ~/projects/apcocsm/code/profiling/workbench && rsync --progress --include='*/' --include='*/*/' --include='timing*.json' --exclude='*' -zavr nikhef:project_atlas/apcocsm_code/profiling/workbench/HWWRun2GGF_a ./ && cd -
"""

basepath = Path.home() / 'projects/apcocsm/code/profiling/workbench/HWWRun2GGF_a'
...
fpgloblist = [basepath.glob('%i.allier.nikhef.nl/*.json' % i)
              for i in range(18623721, 18623744)]
...
drop_meta = ['parallel_interleave', 'seed', 'print_level', 'timing_flag',
             ...
             "workspace_name", "model_config_name", "data_name"]
...
df_totals_ideal = load_timing.estimate_ideal_timing(df_totals_really, groupby=['segment', 'cpu/wall'],
                                                    time_col='time_s')
